...
uint8_t Roomba::SevenSegment(uint8_t number)
{
    if (number < 0) number = 0;
    if (number > 15) number = 15;

    uint8_t segmentData[16] = {
        0x3F/*0*/,
        0x06/*1*/,
        0x5B/*2*/,
        0x4F/*3*/,
...
void Roomba::DigitLEDsRaw(uint8_t d3, uint8_t d2, uint8_t d1, uint8_t d0)
{
    DigitLEDsASCII(SevenSegment(d3), SevenSegment(d2), SevenSegment(d1), SevenSegment(d0));
}

void Roomba::DigitLEDsASCII(uint8_t d3, uint8_t d2, uint8_t d1, uint8_t d0)
{
    _serial->write(163);
    _serial->write(d3);
    _serial->write(d2);
...
void Roomba::SetDayTime(uint8_t day, uint8_t hour, uint8_t minute)
{
	if (day > 7) return;
	if (hour > 23) return;
	if (minute > 59) return;
